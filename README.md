...
│  分布式 KV     │ │  分布式 KV     │ │  分布式 KV      │ │  分布式 KV     │
│  (Raft 副本)   │ │  (Raft 副本)   │ │  (Raft 副本)    │ │  (Raft 副本)   │
└────────┬──────┘ └───────┬───────┘ └────────┬──────┘ └───────┬───────┘
         │                │                  │                 │
┌────────▼────────────────▼──────────────────▼─────────────────▼──────────────┐
│                            PD (Placement Driver)                             │
...
| 数据库 | TiDB (TiKV ×4 + PD) | 分布式 SQL，兼容 MySQL |
...
- **架构拓扑图** — TiDB Server → PD → TiKV×4 的可视化连接关系
...
### 实验五：集群可视化演示（新增）
1. 管理员登录 → 管理后台 → 🖥️ 集群状态
2. 观察架构拓扑图：TiDB → PD → TiKV×4
3. 观察 128 个 Region 均匀分布在 4 个 TiKV 节点上
4. 执行 `docker pause tidb-tikv1`，刷新页面观察节点状态变化
5. 讨论：为什么 2 节点 + 3 副本不能完全容错？3+ 节点如何解决？

### 实验六：性能对比
- 对比 TiDB（4 TiKV）vs 单机 MySQL 的查询性能
...
1. **内存不足**：TiKV 每个节点默认分配 1GB，4 节点 TiDB 集群约需 5.5GB+，确保 Docker 可用内存 ≥ 6GB
...
9. **集群容错**：4 TiKV + 3 副本配置下，可容忍 ≤1 个 TiKV 节点故障而不影响数据读写（Raft 多数派原则）；如需容忍 2 节点故障，需要 ≥5 个 TiKV 节点并将 max-replicas 设为 5
